//
//  stdpnames.h
//  NEST
//
//

#ifndef stdpnames_h
#define stdpnames_h

#include "name.h"

namespace stdpnames {

const Name weight_ref("weight_ref");

const Name tau_plus("tau_plus");
const Name tau_plus_triplet("tau_plus_triplet");
const Name tau_minus("tau_minus");
const Name tau_minus_triplet("tau_minus_triplet");

const Name tau_slow("tau_slow");
const Name tau_ht("tau_ht");
const Name tau_hom("tau_hom");
const Name tau_const("tau_const");

const Name A("A");
const Name B("B");
const Name C("C");
const Name P("P");
const Name WP("WP");

const Name beta("beta");
const Name delta("delta");

const Name Zplus("Zplus");
const Name Zslow("Zslow");
const Name Zminus("Zminus");
const Name Zht("Zht");

const Name Aplus("Aplus");
const Name Aplus_triplet("Aplus_triplet");
const Name Aminus("Aminus");
const Name Aminus_triplet("Aminus_triplet");

const Name Kplus("Kplus");
const Name Kplus_triplet("Kplus_triplet");
const Name Kminus("Kminus");
const Name Kminus_triplet("Kminus_triplet");

const Name nearest_spike("nearest_spike");
}

#endif /* stdpnames_h */
